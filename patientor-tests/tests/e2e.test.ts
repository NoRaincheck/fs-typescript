...
test.describe("Listing patients", () => {
  test("should show column headers Name, Gender and Occupation", async ({ page }) => {
    ...
    await expect(page.getByRole("columnheader", { name: "Name" }))
      .toBeVisible();
    await expect(page.getByRole("columnheader", { name: "Gender" }))
      .toBeVisible();
    await expect(page.getByRole("columnheader", { name: "Occupation" }))
      .toBeVisible();
  });

  test("should list seed patients", async ({ page }) => {
    await page.goto("/");
    await expect(page.getByRole("link", { name: "John McClane" }))
      .toBeVisible();
    await expect(page.getByRole("link", { name: "Martin Riggs" }))
      .toBeVisible();
  });
});
...
test.describe("Showing patient info", () => {
  test("should navigate to patient detail page and show patient information", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /John McClane/ }))
      .toBeVisible();
